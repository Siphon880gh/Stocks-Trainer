...
2. Prefer a clean progress store for path flows: Home **Account** → **Reset path** or clear `localStorage` key `analysis_core_progress_v1`.
...
15. `UF-15` Market Navigator (E10)  
16. `UF-16` News literacy quiz (E10)  
17. `UF-17` Multi-market SAMPLE case (E10)  
18. `UF-18` Step coaching (E11)  
...
**Done (global):** Every UF-01…UF-18 recorded PASS in this loop run (or re-PASS after fixes).
...
1. **Orient**
   ...
   - Confirm preconditions (**Reset path** / path confirm / prior milestones).
...
- **Done(global):** UF-01…UF-18 all PASS this run → STOP with a one-line tally.
