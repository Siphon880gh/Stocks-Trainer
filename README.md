...
Open the app → Dashboard (`/`) → **GOAL_INTAKE · Choose path** → confirm.
...
**Navigate markets:** Dashboard → **MARKET_NAV** / Market header Navigator (E10) → pick type → **VIEW_CHARTS** · **READ_LITERACY** · **DECIDE_CASES**. Until E10.M1 ships, use bottom nav **MARKET** + Class filter.
...
| **Headline decisions** | Chase vs fade / company news on equities | Cases · **company-news** pack | SHORT may show **· LOCKED** when gated |
...
| **Keep progress local** | Name, export/import JSON | Dashboard **ACCOUNT_SHELL** | Import restores; no fake cloud sync |
...
|------|-----------|-----------|-------------|
| 1 | E4.M1 Equities Literacy | Vocabulary | Dashboard **NEXT_STEP** → finish **Equities Literacy** quiz |
| 2 | E4.M2 Statement Snapshot | Statements | **NEXT_STEP** → **Statements Literacy** (snapshot cards) |
| 3 | E4.M0 Chart Soft-Gate | Chart fluency | **NEXT_STEP** / **CHART_GATE** → **Indicators** quiz |
| 4 | E5.M3 Earnings Cases | Earnings decisions | **OPEN_CASES** → earnings pack → **LOCK DECISION · REVEAL** |
| 5 | E5.M2 Company News Cases | Headline decisions | Company-news pack → same decide/reveal loop |

**Done:** Dashboard shows **BEGINNER_EQUITIES_PATH_COMPLETE** (credential copy; progress `5/5`).

### B. Decision Maker Path *(case-heavy)*

**Confirm:** `CONFIRM PATH · DECISION MAKER PATH`

Spine: **earnings → company news → macro intro → combined**.  
You still must pass the **Indicators** chart soft-gate (`UNLOCK_GATE`) before graded cases play.

**Done:** **BADGE · DECISION_MAKER_SEGMENT** / Decision Maker completion copy.

Use Dashboard **RESET_PATH** (confirm dialog) to return to GoalPicker and switch paths.

### C. Market Explorer Path *(SAMPLE multi-market — E10)*

**Confirm:** GoalPicker **Market Explorer** (after E10.M8).
...
|------|------------|---------------|
| **UF-01 First-run** | Path & progress | `/` → GoalPicker → pick path → **CONFIRM PATH** → see **NEXT_STEP** / **UNLOCK_GATE** |
| **UF-02 Equities quiz** | Vocabulary | **NEXT_STEP** → `/training?group=equity-literacy` → finish quiz → Dashboard E4.M1 complete |
| **UF-03 Statements quiz** | Statements | **NEXT_STEP** → financial-literacy quiz (snapshots) → E4.M2 complete |
| **UF-04 Chart soft-gate** | Chart fluency | Indicators quiz; before finish, `/cases` shows gate banner / locked packs |
| **UF-05 Earnings case** | Earnings decisions | `/cases?pack=earnings` → open case → **BUY/SELL/HOLD** → **LOCK DECISION · REVEAL** → debrief |
| **UF-06 Company news** | Headline decisions | `/cases?pack=company-news` → decide/reveal; check **SHORT · LOCKED** when shown |
| **UF-07 Beginner credential** | Full Beginner path | UF-02…UF-06 until `5/5` complete → credential on Dashboard |
| **UF-08 Market SAMPLE** | Browse charts | **MARKET** → Class **Equities** → toggle overlays → Provider SAMPLE/DELAYED |
| **UF-09 Archive theory** | Lookups | **READ_THEORY** / CONFIG → Patterns · Indicators · Literacy tabs |
| **UF-10 Practice Draw** | Muscle memory | **PRACTICE_DRAW** → template → draw → **SUBMIT** → reload shows last attempt |
| **UF-11 Decision Maker** | Macro/combined spine | GoalPicker **Decision Maker** → Indicators gate → earnings → news → macro → combined |
| **UF-12 Account / sync stub** | Local progress | **ACCOUNT_SHELL** → **SIGN_IN_LOCAL** → **EXPORT_PROGRESS** / **IMPORT_PROGRESS** |
| **UF-13 Lock regression** | No silent skip | Fresh Beginner confirm (no quizzes) → Statements/Indicators **LOCKED**; cases gated |
| **UF-14 Secondary training** | Extra drills | After gate: pack links + **Equity Pack Drills** / **ANSWER SHEET** practice |
| **UF-15 Market Navigator** *(E10)* | Jump market types | Dashboard **MARKET_NAV** → type → VIEW_CHARTS / READ_LITERACY / DECIDE_CASES |
| **UF-16 News literacy** *(E10)* | Headline skill | Training **news-literacy** → finish quiz |
| **UF-17 Multi-market case** *(E10)* | Learn by deciding | Navigator → Futures or Forex **DECIDE_CASES** → BUY/SELL/HOLD → reveal |
| **UF-18 Step coaching** *(E11)* | Fail-and-learn wizard | `/coach` → open session → wrong rewind → success restart |

Detailed click scripts (exact labels for QA): [`QA_User_Flows.md`](./QA_User_Flows.md). E11 coaching QA lands when **E11.M6** ships.
...
| Guided fail/rewind practice on a topic | **Step coaching** (`/coach`) | UF-18 *(after E11)* |
...
| Route | Role |
|-------|------|
| `/` | Dashboard · GoalPicker · path CTA · ACCOUNT_SHELL |
| `/training` | Quizzes (literacy, indicators, pattern drills, answer sheet) |
| `/market` | SAMPLE / DELAYED charts + overlays |
...
Bottom nav: **DASHBOARD** · **LEARN** · **MARKET** · **CONFIG**.
